                             ...
                             uint32_t       info)
{
	...
	if (info & (MAY_BE_REF)) {
		...
		if (!zval_type) {
			zval_type = zend_jit_load_type(llvm_ctx, zval_addr, ssa_var, info);
		}
...
/* {{{ static Value* zend_jit_load_operand_ptr */
static Value* zend_jit_load_operand_ptr(zend_llvm_ctx &llvm_ctx,
                                        zend_uchar     op_type,
                                        znode_op      *op,
                                        int            ssa_var,
                                        uint32_t       info,
                                        ...
                                        Value        **should_free)
{
	...
	if (info & MAY_BE_IN_REG) {
		zv_addr = zend_jit_reload_from_reg(llvm_ctx, ssa_var, info);
	} else if (op_type == IS_VAR) {		
		PHI_DCL(zv_addr, 3);
		PHI_DCL(to_free, 3);
		BasicBlock *bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
		...
		zend_jit_unexpected_br(llvm_ctx,				
			llvm_ctx.builder.CreateICmpEQ(
				llvm_ctx.builder.CreateAlignedLoad(
					zend_jit_refcount_addr(llvm_ctx, zv_addr), 4),
				llvm_ctx.builder.getInt32(1)),
			bb_follow,
			bb_next);
                             ...
                             zend_op          *opline)
{
	...
	if (opline->op1_type == IS_VAR && should_free) {
		...
		zend_jit_zval_ptr_dtor_ex(llvm_ctx, should_free, NULL, OP1_SSA_VAR(), OP1_INFO(), opline->lineno, 0);
		llvm_ctx.builder.CreateBr(bb_skip);
		llvm_ctx.builder.SetInsertPoint(bb_skip);
	}
							  ...
							  zend_uchar         fetch_type)
{
	...
	if  (opline->op1_type == IS_CV) {
		zend_jit_update_reg_value(
			...
			OP1_DEF_SSA_VAR(),
			OP1_DEF_INFO());
	} else if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), op1_addr, NULL, OP1_SSA_VAR(), OP1_INFO(), opline->lineno)) {
		return 0;
	}
										  ...
										  zend_op           *opline)
{
	...
	if (OP1_OP_TYPE() != IS_UNUSED) {
			...
			if (OP2_OP_TYPE() == IS_CONST) {
				switch (Z_TYPE_P(OP2_OP()->zv)) {
					case IS_STRING:
						...
						break;
					case IS_LONG:
						hval = LLVM_GET_LONG(Z_LVAL_P(OP2_OP()->zv));
						break;
					case IS_DOUBLE:
						hval = LLVM_GET_LONG(zend_dval_to_lval(Z_DVAL_P(OP2_OP()->zv)));
						break;
					case IS_NULL:
						str = llvm_ctx.builder.CreateIntToPtr(
		...
		if (OP1_MAY_BE(MAY_BE_STRING)) {
			...
			if (OP2_OP_TYPE() == IS_CONST) {
				if (Z_TYPE_P(OP2_OP()->zv) < IS_STRING /* simple scalar types */
						|| (Z_TYPE_P(OP2_OP()->zv) == IS_STRING /* or numeric string */
							&& IS_LONG == is_numeric_string(
								Z_STRVAL_P(OP2_OP()->zv), Z_STRLEN_P(OP2_OP()->zv), NULL, NULL, 0))) {
					index = LLVM_GET_LONG(_zval_get_long_func(OP2_OP()->zv));
				} else {
					if (opline->extended_value & ZEND_ISSET) {
						PHI_ADD(result, llvm_ctx.builder.getInt32(IS_FALSE));
					} else {
