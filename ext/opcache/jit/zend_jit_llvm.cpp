                                     ...
                                     bool           check_gc)
{
	...
	if (zval_addr && (info & (MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_REF))) {
		...
		if (check_gc) {
			...
			if (info & MAY_BE_REF) {
				//JIT: ZVAL_DEREF(z);
				zval_addr = zend_jit_deref(llvm_ctx, zval_addr, info);
			}
                                        ...
                                        zend_op          *opline)
{
	...
	zend_jit_save_zval_type_info(llvm_ctx, result, llvm_ctx.builder.getInt32(IS_STRING_EX));
}
/* }}} */
                                         ...
                                         zend_op          *opline)
{
	if (expr_op_type == IS_CONST) {
		zval str;
		int use_copy = zend_make_printable_zval(expr_op->zv, &str);
		...
		if (use_copy) {
			...
			*expr_str = llvm_ctx.builder.CreateIntToPtr(
					LLVM_GET_LONG((zend_uintptr_t)Z_STR_P(expr_op->zv)),
					PointerType::getUnqual(llvm_ctx.zend_string_type));
		...
		if (expr_info & MAY_BE_STRING) {
			...
			if (expr_info & (MAY_BE_ANY - MAY_BE_STRING)) {
				...
				*expr_str = zend_jit_load_str(llvm_ctx, expr);
				llvm_ctx.builder.CreateBr(*bb_string);
			} else {
				*expr_str = zend_jit_load_str(llvm_ctx, expr);
				llvm_ctx.builder.CreateBr(*bb_string);
                                                     ...
                                                     zend_op         *opline)
{
	...
	if (dim_op_type == IS_CONST) {
		...
		switch (Z_TYPE_P(dim_op->zv)) {
			case IS_DOUBLE:
				num_index = zend_dval_to_lval(Z_DVAL_P(dim_op->zv));
				goto numeric_dim;
				break;
			case IS_TRUE:
				num_index = 1;
...
numeric_dim:
			...
			case IS_NULL:
				...
				break;
			case IS_STRING:
				if (ZEND_HANDLE_NUMERIC(Z_STR_P(dim_op->zv), num_index)) {
					goto numeric_dim;
				} else {
					PHI_ADD(offset,
							llvm_ctx.builder.CreateIntToPtr(
								LLVM_GET_LONG((zend_uintptr_t)Z_STR_P(dim_op->zv)),
								PointerType::getUnqual(llvm_ctx.zend_string_type)));
				}
				break;
			default:
		...
		if ((dim_info & MAY_BE_LONG)) {
			...
			if ((dim_info & (MAY_BE_ANY - MAY_BE_LONG))) {
				...
				bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				if (!dim_type) {
					dim_type = zend_jit_load_type(llvm_ctx, dim, dim_info);
				}
				zend_jit_expected_br(llvm_ctx,
					llvm_ctx.builder.CreateICmpEQ(
						dim_type,
		...
		if ((dim_info & MAY_BE_STRING)) {
			...
			str = zend_jit_load_str(llvm_ctx, dim);
		...
		if ((dim_info & (MAY_BE_RESOURCE))) {
			...
			res = zend_jit_load_res(llvm_ctx, dim);
                                        ...
                                        zend_op       *opline)
{
	...
	if (dim_op_type == IS_CONST) {
		...
		PHI_ADD(index, LLVM_GET_LONG(zval_get_long(dim_op->zv)));
	} else {
                                                    ...
                                                    zend_bool         *may_threw)
{
	...
	if ((container_info & MAY_BE_ARRAY)) {
		...
		if (item) {
			zend_jit_try_addref(llvm_ctx, item, NULL, IS_VAR, NULL, -1);
			PHI_ADD(ret, item);
			if (!bb_finish) {
				bb_finish = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
			}
	...
	if ((container_info & MAY_BE_STRING)) {
		...
		str = zend_jit_load_str(llvm_ctx, container);
		...
		zend_jit_empty_str(llvm_ctx, tmp);
		zend_jit_save_zval_type_info(llvm_ctx, tmp, llvm_ctx.builder.getInt32(IS_INTERNED_STRING_EX));
		...
		zend_jit_save_zval_str(llvm_ctx, tmp, str);
		zend_jit_save_zval_type_info(llvm_ctx, tmp, llvm_ctx.builder.getInt32(IS_STRING_EX));
		PHI_ADD(ret, tmp);
		if (!bb_finish) {
			bb_finish = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
		}
	...
	if ((container_info & MAY_BE_OBJECT)) {
		...
		*may_threw = 1;
		...
		zend_jit_try_addref(llvm_ctx, rv, NULL, IS_VAR, NULL, -1);
                                    ...
                                    zend_bool          *may_threw)
{
	...
	if (op1_info & MAY_BE_OBJECT) {
		if (IS_CUSTOM_HANDLERS(op1_scope)) {
			if (op1_info & (MAY_BE_ANY - MAY_BE_OBJECT)) {
				...
				bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				op1_type = zend_jit_load_type(llvm_ctx, op1_addr, op1_info);
				...
				obj = zend_jit_load_obj(llvm_ctx, op1_addr);
				handlers = zend_jit_load_obj_handlers(llvm_ctx, obj);
	...
	if (op2_info & MAY_BE_OBJECT) {
		if (IS_CUSTOM_HANDLERS(op2_scope)) {
			...
			if (op2_info & (MAY_BE_ANY - MAY_BE_OBJECT)) {
				...
				BasicBlock *bb_has_do_op = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				op2_type = zend_jit_load_type(llvm_ctx, op2_addr, op2_info);
				...
				obj = zend_jit_load_obj(llvm_ctx, op2_addr);
				handlers = zend_jit_load_obj_handlers(llvm_ctx, obj);
				do_op = zend_jit_load_obj_handler(llvm_ctx, handlers, do_operation);
	...
	if (bb_op1_string) {
		...
		if (bb_op2_copy) {
			...
			PHI_ADD(op1_str, zend_jit_load_str(llvm_ctx, op1_addr));
	...
	if (bb_op1_copy) {
		...
		if (op1_op_type == result_op_type) {
			...
			zend_jit_zval_dtor_ex(llvm_ctx, op1_addr, op1_type, op1_info, opline->lineno);
	...
	if (bb_do_concat) {
		...
		if ((op1_op_type == result_op_type) && (op1_info & (MAY_BE_RC1|MAY_BE_RCN))) {
			...
			op1_type_info = zend_jit_load_type_info_c(llvm_ctx, op1_addr, op1_op_type, op1_op, op1_info);
			zend_jit_unexpected_br(
					llvm_ctx,
					llvm_ctx.builder.CreateICmpNE(
						llvm_ctx.builder.CreateAnd(
		...
		llvm_ctx.builder.CreateMemCpy(
				llvm_ctx.builder.CreateGEP(
					zend_jit_load_str_val(llvm_ctx, result_str),
					op1_str_len),
				zend_jit_load_str_val(llvm_ctx, op2_str),
				op2_str_len, 1);
		llvm_ctx.builder.CreateAlignedStore(
				llvm_ctx.builder.getInt8(0),
				llvm_ctx.builder.CreateGEP(
					zend_jit_load_str_val(llvm_ctx, result_str),
					result_str_len), 1);
		zend_jit_save_zval_str(llvm_ctx, result_addr, result_str);
		zend_jit_save_zval_type_info(llvm_ctx, result_addr, llvm_ctx.builder.getInt32(IS_STRING_EX));
	...
	if (op1_orig_addr && !zend_jit_free_operand(llvm_ctx, op1_op_type, op1_orig_addr, NULL, op1_info, opline->lineno)) {
	   	return 0;
	}
	if (op2_orig_addr && !zend_jit_free_operand(llvm_ctx, op2_op_type, op2_orig_addr, NULL, op2_info, opline->lineno)) {
	   	return 0;
	}
                                      ...
                                      zend_bool         *may_threw)
{
	...
	if (OP1_DATA_OP_TYPE() == IS_TMP_VAR) {
		...
		Value *real_val;
		Value *val_info = zend_jit_load_type_info_c(llvm_ctx, value, IS_CONST, OP1_DATA_OP(), OP1_DATA_INFO());
		BasicBlock *bb_copy = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
		BasicBlock *bb_cont = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
		PHI_DCL(real_val, 2);
		...
		zend_jit_try_addref(llvm_ctx, value, NULL, OP1_DATA_OP_TYPE(), OP1_DATA_OP(), OP1_DATA_INFO());
	}
	...
	if (opcode == ZEND_ASSIGN_OBJ) {
		...
		if (IS_CUSTOM_HANDLERS(scope)) {
			...
			if (OP1_DATA_OP_TYPE() == IS_CONST) {
				zend_jit_zval_ptr_dtor_ex(llvm_ctx, value, NULL, OP1_DATA_INFO(), opline->lineno, 0);
			}
	...
	if (result /* && !EG(exception_op) */) {
		zend_jit_copy_value(llvm_ctx,
				...
				OP1_DATA_SSA_VAR(),
				OP1_DATA_INFO());
		zend_jit_try_addref(llvm_ctx, result, NULL, OP1_DATA_OP_TYPE(), OP1_DATA_OP(), OP1_DATA_INFO());
	}
	...
	if (OP1_DATA_OP_TYPE() == IS_VAR) {
		if (!zend_jit_free_operand(llvm_ctx,
					OP1_DATA_OP_TYPE(), value, NULL, OP1_DATA_INFO(), opline->lineno)) {
			/* return 0; */
		}
	}
                           ...
                           int               expected_branch)
{
	...
	if (OP1_OP_TYPE() == IS_CONST) {
		...
		llvm_ctx.builder.CreateBr(
			zval_is_true(OP1_OP()->zv) ? bb_true : bb_false);
		return 1;
	...
	if (OP1_MAY_BE(MAY_BE_ANY - (MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_LONG | MAY_BE_DOUBLE))) {
		Value *cmp = llvm_ctx.builder.CreateICmpNE(
				...
				llvm_ctx.builder.getInt32(0));
		if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), orig_zval_addr, zval_type, OP1_INFO() & ~(MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_LONG | MAY_BE_DOUBLE), opline->lineno)) {
			return 0;
		}
                        ...
                        int               expected_branch)
{
	...
	if (bb_long_long) {
		...
		if (opline->opcode != ZEND_CASE && OP1_OP_TYPE() == IS_VAR /*???&& !OP1_MAY_BE(MAY_BE_TMP_ZVAL)*/) {
			if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), orig_op1_addr, NULL, OP1_INFO(), opline->lineno)) {
				return 0;
			}
		}
		if (OP2_OP_TYPE() == IS_VAR /*???&& !OP2_MAY_BE(MAY_BE_TMP_ZVAL)*/) {
			if (!zend_jit_free_operand(llvm_ctx, OP2_OP_TYPE(), orig_op2_addr, NULL, OP2_INFO(), opline->lineno)) {
				return 0;
			}
		}
		if (bb_false && bb_true) {
...
//???				op2_addr = zend_jit_reload_from_reg(llvm_ctx, OP2_OP_TYPE(), OP2_OP(), OP2_SSA_VAR(), OP2_INFO(), (OP1_MAY_BE(MAY_BE_IN_REG) && OP1_OP_TYPE() == IS_CV) ? 2 : 1);
//???			}
...
//???				} else {
					...
					zend_jit_save_zval_type_info(llvm_ctx,
						...
						llvm_ctx.builder.getInt32(IS_FALSE));
//???				}
				if (orig_op1_addr && op1_op_type == IS_VAR && op1_addr != result_addr /*???&& !(op1_info & MAY_BE_TMP_ZVAL)*/) {
					if (!zend_jit_free_operand(llvm_ctx, op1_op_type, orig_op1_addr, NULL, op1_info, lineno)) {
					   	return 0;
					}
				}
				if (orig_op2_addr && op2_op_type == IS_VAR /*???&& !(op2_info & MAY_BE_TMP_ZVAL)*/) {
					if (!zend_jit_free_operand(llvm_ctx, op2_op_type, orig_op2_addr, NULL, op2_info, lineno)) {
						return 0;
					}
				}
				if (!bb_finish) {
...
//???		}
		...
		if (op1_addr != result_addr /*???&& !(op1_info & MAY_BE_IN_REG)*/) {
			if (orig_op1_addr && !zend_jit_free_operand(llvm_ctx, op1_op_type, orig_op1_addr, NULL, op1_info, lineno)) {
				return 0;
			}
		}
//???		if (!(op2_info & MAY_BE_IN_REG)) {
			if (orig_op2_addr && !zend_jit_free_operand(llvm_ctx, op2_op_type, orig_op2_addr, NULL, op2_info, lineno)) {
				return 0;
			}
//???		}
...
//???	} else { 
		...
		op1_addr = zend_jit_load_operand(llvm_ctx,
				OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO(), 0, opline);
		op1_type = zend_jit_load_type_info_c(llvm_ctx, op1_addr, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
		...
		if (OP1_OP_TYPE() == IS_VAR || OP1_OP_TYPE() == IS_CV) {
			if (OP1_MAY_BE(MAY_BE_REF)) {
				...
				Value *ref = zend_jit_load_reference(llvm_ctx, counted);
				Value *ref_type = zend_jit_load_type_info_c(llvm_ctx, ref, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
				zend_jit_copy_value(llvm_ctx, ret, -1,
						ref, ref_type, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO());
				zend_jit_try_addref(llvm_ctx, ref, ref_type, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
				// INFO changed to IS_OBJECT to avoid Z_REFCOUNTED() check
				if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), op1_addr, op1_type, MAY_BE_OBJECT, opline->lineno)) return 0;
				if (bb_noref) {
					if (!bb_finish) {
						bb_finish = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
					}
		...
		if (OP1_MAY_BE(MAY_BE_RC1 | MAY_BE_RCN)) {
			...
			if (OP1_OP_TYPE() == IS_CONST) {
				if (UNEXPECTED(Z_COPYABLE_P(OP1_OP()->zv))) {
                               ...
                               zend_op          *opline)
{
	...
	if (OP1_OP_TYPE() == IS_UNUSED) {
		...
		zend_jit_save_zval_type_info(llvm_ctx, result_addr, llvm_ctx.builder.getInt32(IS_INTERNED_STRING_EX));
	}
	if (opline->opcode == ZEND_ADD_CHAR) {
		zend_jit_add_char_to_string(llvm_ctx, result_addr, result_addr, (char)Z_LVAL_P(OP2_OP()->zv), opline);
	} else if (OP2_OP_TYPE() == IS_CONST) {
		zend_jit_add_string_to_string(llvm_ctx, result_addr, result_addr, Z_STR_P(OP2_OP()->zv), opline);
	} else {
		...
		if (bb_string) {
			llvm_ctx.builder.SetInsertPoint(bb_string);
			zend_jit_add_var_to_string(llvm_ctx, result_addr, result_addr, op2_str, opline);
			if (bb_follow) {
				llvm_ctx.builder.CreateBr(bb_follow);
			}
		}
		...
		if (zend_jit_free_operand(llvm_ctx, OP2_OP_TYPE(), op2, op2_type, OP2_INFO(), opline->lineno) == 0) {
			return 0;
		}
	}
                           ...
                           zend_op          *opline)
{
	...
	//JIT: var_ptr = GET_OP1_ZVAL_PTR_PTR(BP_VAR_RW);
 	Value *op1_addr = zend_jit_load_operand_addr(llvm_ctx,
 		OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO(), BP_VAR_RW, opline);
	Value *op1_type_info = NULL;
	BasicBlock *bb_finish = NULL;
	Value *res;
	...
	if (OP1_MAY_BE(MAY_BE_ANY - MAY_BE_LONG)) {
		if (opline->opcode == ZEND_PRE_INC || opline->opcode == ZEND_PRE_DEC) {
			...
			zend_jit_separate_zval_noref(llvm_ctx, op1_addr, NULL, OP1_OP_TYPE(), OP1_OP(), OP1_INFO(), opline);
			...
			if (RETURN_VALUE_USED(opline)) {
				...
				res = zend_jit_load_slot(llvm_ctx, RES_OP()->var);
				op1_type_info = zend_jit_load_type_info_c(llvm_ctx, op1_addr, OP1_OP_TYPE(), OP1_OP(), OP1_DEF_INFO());
				zend_jit_copy_value(llvm_ctx, res, -1,
						op1_addr, op1_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_DEF_SSA_VAR(), OP1_DEF_INFO());
				zend_jit_try_addref(llvm_ctx, res, op1_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_DEF_INFO());
			}
			...
			if (OP1_MAY_BE(MAY_BE_REF)) {
				...
				Value *ref_type_info = zend_jit_load_type_info_c(llvm_ctx, ref, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
				...
				zend_jit_zval_copy_ctor(llvm_ctx, res, ref_type_info,
						OP1_OP_TYPE(), OP1_OP(), OP1_INFO(), opline);
				if (!bb_op) {
...
//???		return 1;
	...
	Value *arg_addr = zend_jit_GEP(
						...
						llvm_ctx.zval_ptr_type);
	//JIT: EX(call)->num_args = OP2_OP()->num;
	llvm_ctx.builder.CreateAlignedStore(
	...
	if (OP1_OP_TYPE() == IS_CONST) {
		if (UNEXPECTED(Z_OPT_COPYABLE_P(OP1_OP()->zv))) {
...
//???		return 1;
	...
	if (OP1_MAY_BE(MAY_BE_REF)) {
		...
		zend_jit_copy_value(llvm_ctx, arg_addr, -1,
			ref, ref_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO());
		zend_jit_try_addref(llvm_ctx, ref, ref_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
	...
	if (OP1_MAY_BE(MAY_BE_RC1|MAY_BE_RCN)) {
		...
		if (OP1_OP_TYPE() == IS_CV) {
			...
			zend_jit_try_addref(llvm_ctx, op1_addr, op1_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
		}
		if (bb_finish) {
			llvm_ctx.builder.CreateBr(bb_finish);
		}
							  ...
							  zend_uchar         fetch_type)
{
	...
	if (ret) {
		if (opline->extended_value != 0) {
			zend_jit_make_ref(llvm_ctx, ret, NULL, -1);
		}
	...
	if (bb_new_element) {
		llvm_ctx.builder.SetInsertPoint(bb_new_element);
		if (opline->extended_value != 0) {
			zend_jit_make_ref(llvm_ctx, new_elem, NULL, MAY_BE_NULL);
		}
                           ...
                           zend_op        *opline)
{
	...
	if (OP1_OP_TYPE() == IS_CONST) {
		...
		if (str_len){
			zend_jit_save_zval_lval(llvm_ctx, result, str_len);
			zend_jit_save_zval_type_info(llvm_ctx, result, llvm_ctx.builder.getInt32(IS_LONG));
		}
	} else {
		...
		if (OP1_INFO() & MAY_BE_STRING) {
			if (OP1_INFO() & (MAY_BE_ANY - MAY_BE_STRING)) {
				...
				if (!val_type) {
					val_type = zend_jit_load_type(llvm_ctx, val_ptr, OP1_INFO());
				}
			...
			PHI_ADD(str_len, zend_jit_load_str_len(llvm_ctx,
						zend_jit_load_str(llvm_ctx, val_ptr)));
			bb_finish = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
			llvm_ctx.builder.CreateBr(bb_finish);
		}
		...
		if (OP1_INFO() & MAY_BE_LONG) {
			...
			str = zend_jit_long_to_str(llvm_ctx, zend_jit_load_lval(llvm_ctx, val_ptr));
		...
		if (bb_finish) {
			...
			zend_jit_save_zval_lval(llvm_ctx, result, len);
			zend_jit_save_zval_type_info(llvm_ctx, result, llvm_ctx.builder.getInt32(IS_LONG));
		...
		if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), val_ptr, NULL, OP1_INFO(), opline->lineno)) {
			return 0;
		}
                           ...
                           zend_op           *opline)
{
	...
	op1 = zend_jit_deref(llvm_ctx, op1_addr, OP1_INFO());
	op2 = zend_jit_deref(llvm_ctx, op2_addr, OP2_INFO());
                               ...
                               zend_op           *opline)
{
	...
	if (OP1_OP_TYPE() != IS_CV || OP1_MAY_BE(MAY_BE_REF)) {
		return zend_jit_handler(llvm_ctx, opline);
	}
...
#if 0
	//JIT: if (UNEXPECTED(Z_ISREF_P(object_ptr)) && Z_TYPE_P(Z_REFVAL_P(object_ptr)) == IS_OBJECT)
	if (OP1_MAY_BE(MAY_BE_REF)) {
		BasicBlock *bb_object;
		BasicBlock *bb_noref = NULL;
		if (OP1_MAY_BE(MAY_BE_RC1|MAY_BE_RCN)) {
			BasicBlock *bb_ref = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
			bb_noref = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
	...
	if (OP1_MAY_BE(MAY_BE_OBJECT)) {
		...
		zend_jit_assign_to_object(
				...
				OP1_INFO(),
				OP1_OP_TYPE(),
				dim? dim : llvm_ctx.builder.CreateIntToPtr(LLVM_GET_LONG(0), llvm_ctx.zval_ptr_type),
				OP2_SSA_VAR(),
				OP2_INFO(),
				OP2_OP(),
				OP2_OP_TYPE(),
	...
	if (OP1_MAY_BE(MAY_BE_ANY - MAY_BE_OBJECT)) {
		...
		result = zend_jit_load_slot(llvm_ctx, (opline + 1)->op2.var);
		ret = zend_jit_fetch_dimension_address(
				llvm_ctx,
				NULL,
				result,
		...
		if (ret) {
			...
			if (OP1_DATA_OP_TYPE() == IS_VAR) {
				if (!zend_jit_free_operand(llvm_ctx,
							OP1_DATA_OP_TYPE(), value, NULL, OP1_DATA_INFO(), opline->lineno, 1)) {
					return 0;
				}
			}
	...
	if (dim && !zend_jit_free_operand(llvm_ctx, OP2_OP_TYPE(), dim, NULL, OP2_INFO(), opline->lineno)) {
	   	return 0;
	}
                               ...
                               zend_op           *opline)
{
	...
	if (!zend_jit_free_operand(llvm_ctx, OP2_OP_TYPE(), property, NULL, OP2_INFO(), opline->lineno)) {
	   	return 0;
	}
                              ...
                              zend_uchar         assign_type)
{
	...
	int same_cvs = 0;
	uint32_t var_info;
	uint32_t var_ssa_var;
	zend_uchar var_op_type;
	znode_op *var_op;
	uint32_t val_info;
	...
	if (OP1_OP_TYPE() == IS_VAR || opline->extended_value == ZEND_ASSIGN_OBJ ||
		...
		case ZEND_ASSIGN_DIM:
			{
				...
				if (OP1_MAY_BE(MAY_BE_ANY - MAY_BE_OBJECT)) {
					...
					Value *result = zend_jit_load_slot(llvm_ctx, (opline + 1)->op2.var);
					...
					if (op2_addr) {
						dim = zend_jit_deref(llvm_ctx, op2_addr, OP2_INFO());
					}
	...
	if (do_assign) {
		...
		zend_jit_separate_zval_noref(llvm_ctx, var_ptr, var_type, var_op_type, var_op, var_info, opline);
		...
		if (var_info & MAY_BE_OBJECT) {
			...
			if (IS_CUSTOM_HANDLERS(scope)) {
				...
				if (var_info & (MAY_BE_ANY - MAY_BE_OBJECT)) {
					BasicBlock *bb_object = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
					if (!var_type) {
						var_type = zend_jit_load_type(llvm_ctx, var_ptr, var_info);
					}
					zend_jit_expected_br(
							llvm_ctx,
							llvm_ctx.builder.CreateICmpEQ(
				...
				obj = zend_jit_load_obj(llvm_ctx, var_ptr);
                               ...
                               zend_op          *opline)
{
	...
	if (func && func->type == ZEND_INTERNAL_FUNCTION) {
		...
		Value *zv_addr = zend_jit_hash_find(llvm_ctx,
			...
			llvm_ctx.builder.CreateIntToPtr(
				LLVM_GET_LONG((zend_uintptr_t)(Z_STR_P(OP2_OP()->zv))),
				PointerType::getUnqual(llvm_ctx.zend_string_type)));
		...
		if (!func) {
			...
			zend_jit_error_noreturn(
				...
				LLVM_GET_CONST_STRING(Z_STRVAL_P(OP2_OP()->zv)));
			//JIT: } else {
			llvm_ctx.builder.SetInsertPoint(bb_found);
		}
                                       ...
                                       zend_op            *opline)
{
	if (num_args != -1) {
		while (num_args > 0) {
			...
			zend_jit_zval_ptr_dtor_ex(llvm_ctx, arg, NULL, -1, opline->lineno, 0);
		}
	} else {
//???..check
		Value *num_args_val =
		...
		var = zend_jit_GEP(
			...
			llvm_ctx.zval_ptr_type);
		//JIT: zval_ptr_dtor_nogc(p);
		zend_jit_zval_ptr_dtor_ex(llvm_ctx, var, NULL, -1, opline->lineno, 0);
		//JIT: } while (var != end);
		PHI_ADD2(var, var);
		zend_jit_expected_br(llvm_ctx,
			llvm_ctx.builder.CreateICmpNE(
			...
			if (src != dst) {
				do {
					...
					zend_jit_copy_value(llvm_ctx, 
						...
						-1);						
					//JIT: ZVAL_UNDEF(src);
					zend_jit_save_zval_type_info(llvm_ctx, src_val, llvm_ctx.builder.getInt32(IS_UNDEF));
					src--;
					dst--;
				} while (src != end);
			}
...
//???..check
		...
		zend_jit_copy_value(llvm_ctx, 
			...
			-1);						
		//JIT: ZVAL_UNDEF(src);
		zend_jit_save_zval_type_info(llvm_ctx, src, llvm_ctx.builder.getInt32(IS_UNDEF));
		//JIT: src--;
		src = zend_jit_GEP(
			llvm_ctx,
			src,
	...
	if (func && num_args != -1) {
		...
		for (i = num_args; i < func->op_array.last_var; i++) {
			...
			Value *var = zend_jit_GEP(
				...
				llvm_ctx.zval_ptr_type);
			//JIT: ZVAL_UNDEF(var);
			zend_jit_save_zval_type_info(llvm_ctx, var, llvm_ctx.builder.getInt32(IS_UNDEF));
		}
	} else {
//???..check
		Value *num_args_val = NULL;
		...
		Value *var =
			llvm_ctx.builder.CreateInBoundsGEP(
				zend_jit_GEP(
					...
					llvm_ctx.zval_ptr_type),
				this_var);
		zend_jit_save_zval_obj(llvm_ctx, var, object);
		zend_jit_save_zval_type_info(llvm_ctx, var, llvm_ctx.builder.getInt32(IS_OBJECT_EX));
	...
	if (!func || func->type == ZEND_INTERNAL_FUNCTION) {
		...
		if (zend_jit_check_type_hints(llvm_ctx, func, func_addr, num_args, call) == 2) {
			...
			if (RETURN_VALUE_USED(opline)) {
				...
				zend_jit_save_zval_type_info(llvm_ctx, ret, llvm_ctx.builder.getInt32(IS_UNDEF));
			}
...
//???..check
		...
		if (!RETURN_VALUE_USED(opline)) {
			...
			Value *ret = zend_jit_load_var(llvm_ctx, RES_OP()->var);
			zend_jit_zval_ptr_dtor_ex(llvm_ctx, ret, NULL, RES_INFO(), opline->lineno, 1); 
		}
                                            ...
                                            zend_op          *opline)
{
	...
	for (uint32_t i = 0 ; i < op_array->last_var; i++) {
		...
		Value *var = zend_jit_GEP(
			...
			(ZEND_CALL_FRAME_SLOT + i) * sizeof(zval),
			llvm_ctx.zval_ptr_type);
		zend_jit_zval_ptr_dtor_ex(llvm_ctx, var, NULL, info, opline->lineno, 1);
	}
	return 1;
}
/* }}} */
                                    ...
                                    zend_op          *opline)
{
	...
	//JIT: if (UNEXPECTED(call->num_args > first_extra_arg)) {
	Value *num_args = 
		llvm_ctx.builder.CreateAlignedLoad(
			zend_jit_GEP(
				llvm_ctx,
...
//TODO: ???
	...
	//JIT: FREE_OP1();
	if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), retval, NULL, OP1_INFO(), opline->lineno)) {
		return 0;
	}
	llvm_ctx.builder.CreateBr(bb_end);
	...
	if (OP1_OP_TYPE() == IS_CONST || OP1_OP_TYPE() == IS_TMP_VAR) {
		...
		zend_jit_copy_value(llvm_ctx, return_value, 0,
				retval, NULL, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO());
		if (OP1_OP_TYPE() == IS_CONST) {
			if (Z_OPT_COPYABLE_P(OP1_OP()->zv)) {
		...
		if (OP1_MAY_BE(MAY_BE_REF)) {
			if (OP1_MAY_BE(MAY_BE_RC1|MAY_BE_RCN)) {
				...
				retval_type_info = zend_jit_load_type_info_c(llvm_ctx, retval, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
				BasicBlock *bb_ref = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				bb_no_ref = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				zend_jit_unexpected_br(llvm_ctx,
					llvm_ctx.builder.CreateICmpEQ(
			...
			Value *counted = zend_jit_load_counted(llvm_ctx, retval);
			Value *ref_addr = zend_jit_load_reference(llvm_ctx, counted);
			Value *ref_type_info = zend_jit_load_type_info(llvm_ctx, ref_addr);
			zend_jit_copy_value(llvm_ctx, return_value, 0,
				ref_addr, ref_type_info, OP1_OP_TYPE(), OP1_OP(), -1, OP1_INFO());
			zend_jit_try_addref(llvm_ctx, return_value, ref_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
			if (OP1_OP_TYPE() == IS_VAR) {
				//JIT: FREE_OP1_IF_VAR();
				if (!zend_jit_free_operand(llvm_ctx, OP1_OP_TYPE(), retval, NULL, OP1_INFO(), opline->lineno)) {
				   	return 0;
		...
		if (OP1_MAY_BE(MAY_BE_RC1|MAY_BE_RCN)) {
			...
			if (OP1_OP_TYPE() == IS_CV) {
				...
				zend_jit_try_addref(llvm_ctx, return_value, retval_type_info, OP1_OP_TYPE(), OP1_OP(), OP1_INFO());
			}
			llvm_ctx.builder.CreateBr(bb_end);
		}
	}
                         ...
                         zend_op          *opline)
{
	...
	//JIT: if (UNEXPECTED(arg_num > EX(num_args))) {
	Value *num_args = llvm_ctx.builder.CreateAlignedLoad(
		zend_jit_GEP(
			llvm_ctx,
			llvm_ctx._execute_data,
	...
	if (opline->opcode == ZEND_RECV_INIT) {
		...
		zend_jit_copy_value(llvm_ctx, param, -1,
			llvm_ctx.builder.CreateIntToPtr(
				LLVM_GET_LONG((zend_uintptr_t)OP2_OP()->zv),
				llvm_ctx.zval_ptr_type),
			NULL,
			OP2_OP_TYPE(),
			OP2_OP(),
...
//			}
			...
			switch (opline->opcode) {
				...
				case ZEND_OP_DATA:
					...
					break;
				case ZEND_JMP:
					llvm_ctx.builder.CreateBr(llvm_ctx.bb_labels[OP1_OP()->jmp_addr - op_array->opcodes]);
					llvm_ctx.valid_opline = 0;
					break;
				case ZEND_JMPZ:
					if (!zend_jit_jmpznz(
							...
							op_array,
							opline,
							llvm_ctx.bb_labels[OP2_OP()->jmp_addr - op_array->opcodes],
...
//					break;
				...
				case ZEND_CONT:
					...
					if (OP2_OP_TYPE() == IS_CONST &&
						...
						int nest_levels = Z_LVAL_P(OP2_OP()->zv);
...
//					break;
				...
				case ZEND_FE_RESET:
					if (!zend_jit_handler(llvm_ctx, opline)) return 0;
					if (!zend_jit_cond_jmp(llvm_ctx, OP2_OP()->jmp_addr, llvm_ctx.bb_labels[block[b + 1].start])) return 0;
					break;
				case ZEND_FE_FETCH:
					if (!zend_jit_handler(llvm_ctx, opline)) return 0;
					if (!zend_jit_cond_jmp(llvm_ctx, OP2_OP()->jmp_addr, llvm_ctx.bb_labels[block[b + 1].start])) return 0;
