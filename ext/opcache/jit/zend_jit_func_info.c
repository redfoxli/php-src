...
typedef uint32_t (*info_func_t)(const zend_jit_call_info *call_info);
...
static uint32_t zend_jit_strlen_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static uint32_t zend_jit_dechex_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static uint32_t zend_jit_range_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
	...
	if (call_info->caller_init_opline->extended_value == call_info->num_args &&
		...
		uint32_t t1 = ssa_op1_info(call_info->caller_op_array, call_info->arg_info[0].opline);
		uint32_t t2 = ssa_op1_info(call_info->caller_op_array, call_info->arg_info[1].opline);
		uint32_t t3 = 0;
		uint32_t tmp = MAY_BE_DEF | MAY_BE_RC1 | MAY_BE_ARRAY | MAY_BE_ARRAY_KEY_LONG;

		if (call_info->num_args == 3) {
			t3 = ssa_op1_info(call_info->caller_op_array, call_info->arg_info[2].opline);
		}
...
static uint32_t zend_jit_is_type_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static uint32_t zend_jit_l_ss_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static uint32_t zend_jit_lb_ssn_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static uint32_t zend_jit_b_s_info(const zend_jit_call_info *call_info)
{
	zend_jit_func_info *info = JIT_DATA(call_info->caller_op_array);
...
static const func_info_t func_infos[] = {
	...
	FC("strlen",                  zend_jit_strlen_info),
	FC("strcmp",                  zend_jit_l_ss_info),
	FC("strncmp",                 zend_jit_lb_ssn_info),
	FC("strcasecmp",              zend_jit_l_ss_info),
	FC("strncasecmp",             zend_jit_lb_ssn_info),
	F1("each",                    MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_ARRAY | MAY_BE_ARRAY_KEY_STRING | MAY_BE_ARRAY_OF_REF | MAY_BE_ARRAY_OF_ANY),
	F1("error_reporting",         MAY_BE_NULL | MAY_BE_LONG),
	F1("define",                  MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_NULL), // TODO: inline
	FC("defined",                 zend_jit_b_s_info), // TODO: inline
	...
	F1("class_exists",            MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_TRUE),
 	...
 	FC("function_exists",         zend_jit_b_s_info), // TODO: inline
	...
	F1("debug_backtrace",         MAY_BE_NULL | MAY_BE_ARRAY | MAY_BE_ARRAY_KEY_LONG | MAY_BE_ARRAY_OF_ARRAY),
 	...
 	FC("extension_loaded",        zend_jit_b_s_info),
	...
	FC("dechex",                       zend_jit_dechex_info),
...
# endif
	...
	FC("is_null",                      zend_jit_is_type_info),
	F1("is_resource",                  MAY_BE_FALSE | MAY_BE_TRUE), // TODO: inline with support for closed resources
	FC("is_bool",                      zend_jit_is_type_info),
	FC("is_long",                      zend_jit_is_type_info),
	FC("is_float",                     zend_jit_is_type_info),
	FC("is_int",                       zend_jit_is_type_info),
	FC("is_integer",                   zend_jit_is_type_info),
	FC("is_double",                    zend_jit_is_type_info),
	FC("is_real",                      zend_jit_is_type_info),
	F1("is_numeric",                   MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_TRUE),
	FC("is_string",                    zend_jit_is_type_info),
	FC("is_array",                     zend_jit_is_type_info),
...
uint32_t zend_jit_get_func_info(const zend_jit_call_info *call_info)
{
	...
	if (call_info->callee_func->type == ZEND_INTERNAL_FUNCTION) {
		...
		zend_jit_func_info *info = JIT_DATA((zend_op_array*)call_info->callee_func);
		if (info) {
